"""
Construir uma tabela de conversão de graus
Fahreinheit para Celsius, para todos os
Fahrenheit em um intervalo informado pelo
usuário. Obs: C = (F - 32) * 5 / 9
"""
# f_1 = int(input("De o primeiro numero do intervalo em °F: "))
# f_2 = int(input("De o segundo numero do intervalo em °F: "))
# if f_1 > f_2:
#     f_1, f_2 = f_2, f_1

# for f in range(f_1, f_2 + 1):
#     c = (f - 32)*(5/9)
#     print(f"{f:2}°F = {c:.2f}ºC")


"""agora usando função"""
def conversão_temperatura_intervalo(minima, maxima):
    tabela = {}
    for f in range(minima, maxima + 1):
        # c = (f - 32) * (5 / 9)
        tabela[f] = {f"{f:2}°F" : f"{(f - 32) * (5 / 9):.2f}ºC"}
    return tabela

print(conversão_temperatura_intervalo(1, 10))